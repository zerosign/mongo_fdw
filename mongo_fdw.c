...
static void MongoBeginForeignScan(ForeignScanState *scanState,
                                  int executorFlags);
...
static void FillTupleSlot(const bson_t *bsonDocument,
                          const char *bsonDocumentKey, HTAB *columnMappingHash,
...
/*
 * MongoGetForeignRelSize obtains relation size estimates for mongo foreign
 * table.
 */
static void MongoGetForeignRelSize(PlannerInfo *root, RelOptInfo *baserel,
                                   Oid foreignTableId) {
  ...
  if (documentCount > 0.0) {
    ...
    double rowSelectivity =
        clauselist_selectivity(root, rowClauseList, 0, JOIN_INNER, NULL);
...
static void MongoGetForeignPaths(PlannerInfo *root, RelOptInfo *baserel,
  ...
  if (documentCount > 0.0) {
    ...
    documentSelectivity =
        clauselist_selectivity(root, opExpressionList, 0, JOIN_INNER, NULL);
     ...
     */
    documentWidth =
        get_relation_data_width(foreignTableId, baserel->attr_widths);
    ...
    cpuCostPerRow =
        (cpu_tuple_cost * MONGO_TUPLE_COST_MULTIPLIER) + tupleFilterCost;
    totalCpuCost =
        (cpuCostPerDoc * documentCount) + (cpuCostPerRow * inputRowCount);
...
static ForeignScan *MongoGetForeignPlan(PlannerInfo *root, RelOptInfo *baserel,
  ...
  foreignScan =
      make_foreignscan(targetList, restrictionClauses, scanRangeTableIndex,
...
static void MongoBeginForeignScan(ForeignScanState *scanState,
  ...
  if (connectStatus != MONGO_OK) {
    ...
    ereport(ERROR,
            (errmsg("could not connect to %s:%d", addressName, portNumber),
  ...
  executionState->client = mongoConnection;
...
static void MongoReScanForeignScan(ForeignScanState *scanState) {
  ...
  mongoCursor = mongo_cursor_create();
  mongo_cursor_init(mongoCursor, mongoConnection, namespaceName->data);
  mongo_cursor_set_query(mongoCursor, executionState->query_document);
...
static double ForeignTableDocumentCount(Oid foreignTableId) {
  ...
  status = mongo_connect(mongoConnection, options->address_name,
                         options->port_number);
  if (status == MONGO_OK) {
 ...
 * and types. This table helps us quickly translate BSON_T document key/values
 * to
...
static HTAB *ColumnMappingHash(Oid foreignTableId, List *columnList) {
  ...
  columnMappingHash =
      hash_create("Column Mapping Hash", hashTableSize, &hashInfo,
...
static void FillTupleSlot(const bson_t *bsonDocument,
  ...
  while (bson_iterator_next(&bsonIterator)) {
    ...
    if (bsonType == BSON_OBJECT) {
      ...
      FillTupleSlot(&subObject, bsonFullKey, columnMappingHash, columnValues,
                    columnNulls);
      continue;
    }
    ...
    if (OidIsValid(columnArrayTypeId)) {
      ...
      columnValues[columnIndex] =
          ColumnValueArray(&bsonIterator, columnArrayTypeId);
      ...
      columnValues[columnIndex] =
          ColumnValue(&bsonIterator, columnTypeId, columnTypeMod);
      columnNulls[columnIndex] = false;
    }
  }
}
 ...
 * to by the BSON_T iterator, and converts each array element (with matching
 * type)
...
static Datum ColumnValueArray(bson_iter_t *bsonIterator, Oid valueTypeId) {
  ...
  while (bson_iterator_next(&bsonSubIterator)) {
    ...
    if (arrayIndex >= arrayCapacity) {
      ...
      columnValueArray =
          repalloc(columnValueArray, arrayCapacity * sizeof(Datum));
    }

    /* use default type modifier (0) to convert column value */
    columnValueArray[arrayIndex] =
        ColumnValue(&bsonSubIterator, valueTypeId, 0);
    arrayIndex++;
  }
 ...
 * by the BSON_T iterator, and converts this value to the corresponding
 * PostgreSQL
...
static Datum ColumnValue(bson_iter_t *bsonIterator, Oid columnTypeId,
  ...
  case BPCHAROID: {
    ...
    columnValue =
        DirectFunctionCall3(bpcharin, valueDatum, ObjectIdGetDatum(InvalidOid),
  ...
  case VARCHAROID: {
    ...
    columnValue =
        DirectFunctionCall3(varcharin, valueDatum, ObjectIdGetDatum(InvalidOid),
                            Int32GetDatum(columnTypeMod));
    break;
  }
  case TEXTOID: {
  ...
  case NAMEOID: {
    ...
    columnValue =
        DirectFunctionCall3(namein, valueDatum, ObjectIdGetDatum(InvalidOid),
                            Int32GetDatum(columnTypeMod));
    break;
  }
  case DATEOID: {
...
static void MongoFreeScanState(MongoFdwExecState *executionState) {
  ...
  mongo_destroy(executionState->client);
  mongo_dispose(executionState->client);
}
...
static int MongoAcquireSampleRows(Relation relation, int errorLevel,
   ...
   */
  tupleContext =
      AllocSetContextCreate(CurrentMemoryContext, "mongo_fdw temporary context",
  ...
  for (;;) {
    ...
    if (sampleRowCount < targetRowCount) {
      sampleRows[sampleRowCount++] =
          heap_form_tuple(tupleDescriptor, columnValues, columnNulls);
    } else {
      /*
       ...
       */
      if (rowCountToSkip < 0) {
        rowCountToSkip = anl_get_next_S(rowCount, targetRowCount, &randomState);
      }
